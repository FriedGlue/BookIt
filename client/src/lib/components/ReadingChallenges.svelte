...
<div class="flex w-full max-w-7xl flex-col items-center">
	{#each challenges as challenge}
		<div class="mb-16 w-full text-left">
			<div class="mb-4 flex flex-col sm:flex-row items-start justify-between">
				<div>
					<h1 class="text-xl font-bold text-gray-600 md:text-2xl lg:text-4xl">
						{challenge.name}
					</h1>
                ...
                {#if isChallengesPage}
                    ...
                    >
                        ...
                        <button
                            type="submit"
                            class="rounded-full bg-red-500 px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm text-white hover:bg-red-600"
                        >
                            Delete Challenge
                        </button>
                    </form>
			...
			<div class="mt-6 w-full">
				...
				<div class="h-6 sm:h-8 w-full overflow-hidden rounded-full border-2 border-gray-300 bg-gray-200">
					<div
				...
				<div class="mt-2 flex items-center justify-between text-xs sm:text-sm text-gray-600">
				...
				<div class="mt-4 grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4">
					...
					<div class="rounded-lg bg-gray-100 p-3 sm:p-4 text-center">
						<p class="text-lg sm:text-xl md:text-2xl font-bold text-blue-500">
							{challenge.progress.percentage.toFixed(1)}%
						</p>
						<p class="text-xxs sm:text-xs text-gray-600">Complete</p>
					</div>
					...
					<div class="rounded-lg bg-gray-100 p-3 sm:p-4 text-center">
						...
						<p class="text-xxs sm:text-xs text-gray-600">
					...
					<div class="rounded-lg bg-gray-100 p-3 sm:p-4 text-center">
						{#if challenge.progress.rate.status === 'AHEAD'}
							<p
								class="text-lg sm:text-xl md:text-2xl font-bold text-green-500"
							...
							<p class="text-xxs sm:text-xs text-gray-600">Ahead of Schedule</p>
						{:else if challenge.progress.rate.status === 'ON_TRACK'}
							<p class="text-lg sm:text-xl md:text-2xl font-bold text-blue-500">On Track</p>
							<p class="text-xxs sm:text-xs text-gray-600">Keep it up!</p>
						{:else}
							<p
								class="text-lg sm:text-xl md:text-2xl font-bold text-red-500"
								title="Behind by {formatRate(challenge.progress.rate.scheduleDiff, true, challenge.type)} {challenge.progress.rate.unit}"
							>
								{formatRate(challenge.progress.rate.scheduleDiff, false, challenge.type)}
							</p>
							<p class="text-xxs sm:text-xs text-gray-600">Behind Schedule</p>
						{/if}
					</div>
				</div>

				<!-- Reading Challenge Status -->
				<div class="my-12 sm:my-24 mt-4 flex items-center gap-4">
					{#if challenge.progress.rate.status === 'AHEAD'}
						<div class="flex items-center gap-2">
							<div class="h-2 w-2 sm:h-3 sm:w-3 rounded-full bg-green-500"></div>
							<span class="text-xs sm:text-sm text-gray-600">Ahead of Schedule</span>
						</div>
					{:else if challenge.progress.rate.status === 'ON_TRACK'}
						<div class="flex items-center gap-2">
							<div class="h-2 w-2 sm:h-3 sm:w-3 rounded-full bg-blue-500"></div>
							<span class="text-xs sm:text-sm text-gray-600">On Track</span>
						</div>
					{:else}
						<div class="flex items-center gap-2">
							<div class="h-2 w-2 sm:h-3 sm:w-3 rounded-full bg-red-500"></div>
							<span class="text-xs sm:text-sm text-gray-600">Behind Schedule</span>
						</div>
					{/if}
				</div>
			</div>
